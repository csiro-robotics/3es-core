...
public:
  ...
  /// @overload
  Plane(uint32_t id = 0u, const V3Arg &position = V3Arg(0, 0, 0), const V3Arg &normal = DefaultNormal,
        float scale = 1.0f, float normalLength = 1.0f);
  /// Create a plane.
  ...
  Plane(uint32_t id, uint16_t category, const V3Arg &position = V3Arg(0, 0, 0), const V3Arg &normal = DefaultNormal,
        float scale = 1.0f, float normalLength = 1.0f);
...
inline Plane::Plane(uint32_t id, uint16_t category, const V3Arg &position, const V3Arg &normal, float scale,
                    float normalLength)
