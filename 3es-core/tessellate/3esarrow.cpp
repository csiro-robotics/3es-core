...
namespace
{
                  ...
                  const Vector3f arrowAxis)
  {
    if (facets < 3 || cylinderLength <= 0 || arrowLength < 0 || arrowLength <= cylinderLength ||
        headRadius <= 0 || cylinderRadius <= 0 || headRadius <= cylinderRadius)
    {
      return false;
    }
    ...
    for (unsigned i = 0; i < facets; ++i)
    {
      ...
      vertices[vind] = vertices[vind + facets] = Vector3f(headRadius * std::sin(facetAngle),
                                                          headRadius * std::cos(facetAngle), cylinderLength);
      ++vind;
    }
    ...
    {
      const float facetAngle = float(i * 2 * M_PI) / float(facets);
      vertices[vind] = vertices[vind + facets] = Vector3f(cylinderRadius * std::sin(facetAngle),
                                                          cylinderRadius * std::cos(facetAngle), cylinderLength);
      ++vind;
    }
    // Account for having built two vertices per facet above.
    vind += facets;
    ...
    for (unsigned i = 0; i < facets; ++i)
    {
      const float facetAngle = float(i * 2 * M_PI) / float(facets);
      vertices[vind] = vertices[vind + facets] = Vector3f(cylinderRadius * std::sin(facetAngle),
                                                          cylinderRadius * std::cos(facetAngle), 0);
      ++vind;
    }
...
bool tes::arrow::solid(std::vector<Vector3f> &vertices, std::vector<unsigned> &indices,
                       std::vector<Vector3f> &normals, unsigned facets, float headRadius, float cylinderRadius,
                       float cylinderLength, float arrowLength, const Vector3f axis)
{
  return buildArrow(vertices, indices, &normals, facets, headRadius, cylinderRadius, cylinderLength, arrowLength,
                    axis);
}
