...
{
public:
  explicit PickableObject(UnsignedInt id, Shaders::PhongGL &shader, const Color3 &color, GL::Mesh &mesh,
                          Object3D &parent, SceneGraph::DrawableGroup3D &drawables)
    : Object3D{ &parent }
...
PickingExample::PickingExample(const Arguments &arguments)
  : Platform::Application{ arguments, Configuration{}.setTitle("Magnum object picking example") }
  , _framebuffer{ GL::defaultFramebuffer.viewport() }
{
  ...
  _framebuffer.attachRenderbuffer(GL::Framebuffer::ColorAttachment{ 0 }, _color)
    ...
    .mapForDraw({ { Shaders::Pong::ColorOutput, GL::Framebuffer::ColorAttachment{ 0 } },
                  { Shaders::Pong::ObjectIdOutput, GL::Framebuffer::ColorAttachment{ 1 } } });
...
void PickingExample::drawEvent()
{
  ...
  _framebuffer.clearColor(0, Color3{ 0.125f }).clearColor(1, Vector4ui{}).clearDepth(1.0f).bind();
  ...
  GL::AbstractFramebuffer::blit(_framebuffer, GL::defaultFramebuffer, _framebuffer.viewport(),
...
void PickingExample::mouseMoveEvent(MouseMoveEvent &event)
{
  ...
  const Vector2 delta = 3.0f * Vector2{ event.position() - _previousMousePosition } / Vector2{ windowSize() };
...
void PickingExample::mouseReleaseEvent(MouseEvent &event)
{
  ...
  Image2D data = _framebuffer.read(Range2Di::fromSize(fbPosition, { 1, 1 }), { PixelFormat::R32UI });
