...
Shader &PointGeom::setClipPlanes(float near, float far)
{
  _shader->setClipPlanes(near, far);
  return *this;
}
...
{
  setUniform(_tinit_uniform, colour);
  return *this;
}
...
PointGeomProgram &PointGeomProgram::setClipPlanes(float near, float far)
{
  const Magnum::Math::Vector4<float> clip_params = Magnum::Vector4(  //
    near, far,                                                       //
    (near) ? 1 / near : 0,                                           //
    (far) ? 1 / far : 0);
  setUniform(_clip_params_uniform, clip_params);
  return *this;
}
