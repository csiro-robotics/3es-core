...
void Colour::rgbToHsv(float &h, float &s, float &v, const float r, const float g, const float b)
{
  ...
  h = (yellowToMagenta * ((g - b) / delta) + cyanToYellow * ((g - b) / delta) + magentaToCyan * ((g - b) / delta)) *
      60.0f;
}


void Colour::hsvToRgb(float &r, float &g, float &b, const float h, const float s, const float v)
{
...
void Colour::hsvToRgb(uint8_t &r, uint8_t &g, uint8_t &b, const float h, const float s, const float v)
{
  float rf, gf, bf;
