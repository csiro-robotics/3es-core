...
unsigned tesUnrollDisplay(const std::vector<Vector3f> &vertices, const std::vector<unsigned> &indices)
{
  ...
  while (cursor < indices.size())
  {
    ...
    cursor += count;
    TES_TRIANGLES(tesServer, TES_RGB(200, 200, 200), localVertices.data()->v, (unsigned)count,
                  sizeof(*localVertices.data()), SPHERE_ID + shapeCount);
    ++shapeCount;
  }
...
void sphereInitialise(std::vector<Vector3f> &vertices, std::vector<unsigned> &indices, SphereVertexMap *vertexMap)
{
  ...
  {
    TES_TRIANGLES(tesServer, TES_RGB(200, 200, 200), vertices.data()->v, (unsigned)vertices.size(),
                  sizeof(*vertices.data()), indices.data(), (unsigned)indices.size(), SPHERE_ID);
  }
}
